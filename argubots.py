...
class ParallelAgent:
    def __init__(self, name: str, model: str, n: int = 1, **kwargs):
        self.name = name
        self.model = model
        self.n = n
        self.kwargs = kwargs

    async def response(self, dialogue):
        messages = self.format_as_messages(dialogue)

        completions = await asyncio.gather(
            *(self.generate_completion(messages) for _ in range(self.n))
        )
        
        selected_completion = self.select_best_completion(completions)
        return selected_completion['message']['content'].strip()

    async def generate_completion(self, messages):
        response = await self.client.chat.completions.create(
            messages=messages, model=self.model, **self.kwargs
        )
        return response.choices[0]

    def select_best_completion(self, choices):
        best_choice = None
        best_score = float('-inf')

        for choice in choices:
            score = self.evaluate_completion(choice['message']['content'])
            if score > best_score:
                best_score = score
                best_choice = choice

        return best_choice if best_choice else choices[0]

    def evaluate_completion(self, completion):
        ideal_length = 100
        length_score = -abs(len(completion) - ideal_length)
        return length_score
    
parallel = ParallelAgent("Parallel", "davinci", n=5)
